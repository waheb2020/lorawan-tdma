...
public:
  ...
  ApplicationContainer Install (NodeContainer c, uint16_t start_times[]) const;

  ApplicationContainer Install (Ptr<Node> node, uint16_t start_time) const;
...
private:
  Ptr<Application> InstallPriv (Ptr<Node> node, uint16_t start_time) const;
